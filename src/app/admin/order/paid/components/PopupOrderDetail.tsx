        ...
        <DialogContent className="w-full max-w-[90vw] md:max-w-[900px] lg:max-w-[900px] p-3 rounded-[12px]">
            <div className="w-full relative rounded-[12px] bg-white max-h-[80vh] overflow-y-auto overflow-x-hidden flex flex-col p-3 gap-3 text-left text-sm text-black font-inter">
                ...
                <div className="w-full flex flex-row items-center justify-between">
                    <div className="flex flex-row items-center gap-[12px]">
                        ...
                        <Badge
                            ...
                            order.trang_thai_don_hang.toLowerCase().includes("chờ")
                                ? "pending"
                                : order.trang_thai_don_hang.toLowerCase().includes("đã")
                                ? "paid"
                                : "default"
                            }
                        >
                    ...
                    <div className="w-[20px] h-[20px] cursor-pointer" onClick={() => onOpenChange(false)}>
                    </div>
                </div>
                    ...
                    <div className="w-full md:w-1/2 flex flex-col gap-3">
                        ...
                        <div className="w-full flex flex-row items-center justify-between gap-3">
                            <div className="flex flex-col gap-1">
                                ...
                                <strong className="text-xs">
                                    Mã đơn hàng: <span className="font-normal">{order.don_hang_id}</span>
                                </strong>
                                <strong className="text-xs">
                                    Tên thu ngân: <span className="font-normal">{order.thu_ngan?.ho_ten}</span>
                                </strong>
                                <strong className="text-xs">
                                    Thời gian thanh toán: <span className="font-normal">{new Date(order.thoi_gian_thanh_toan).toLocaleString("vi-VN")}</span>
                                </strong>
                            </div>
                            <Image
                        ...
                        <div className="flex flex-col gap-3 text-xs">
                            <div className="flex flex-row w-full justify-between gap-3 grid grid-cols-3">
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <strong>{new Date(order.thoi_gian_dat).toLocaleString("vi-VN")}</strong>
                                </div>
                            </div>
                    ...
                    <div className="w-full md:w-1/2 flex flex-col gap-3">
                                ...
                                <div className="space-y-3 mt-3">
                                    {order.chi_tiet_theo_ngay?.map((day: any, idx: number) => (
                                        <div key={idx} className="border p-2 rounded">
                                            <div className="font-bold">{day.ngay_giao_formatted}</div>
                                            {day.items.map((it: any, i: number) => (
                                            <div key={i} className="flex justify-between text-xs">
                                                <span>
                                                {it.ten_mon_an} x{it.so_luong}
                                                </span>
                                                <span>{parseInt(it.thanh_tien).toLocaleString("vi-VN")} đ</span>
                                            </div>
                                            ))}
                                        </div>
                                    ))}
                        ...
                        <div className="bg-white p-2">
                            <div className="flex justify-between items-center text-sm">
                                ...
                                <strong>{parseInt(order.tong_tien).toLocaleString("vi-VN")} đ</strong>
                            </div>
                        </div>
                    </div>
                </div>
