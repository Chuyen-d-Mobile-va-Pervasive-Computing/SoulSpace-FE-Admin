...
export function DataTablePagination<TData>({
  ...
  return (
    <div className="flex flex-row-reverse items-end justify-between pt-[10px]">
        ...
        <div className="flex items-center gap-1">
          ...
          >
            {Array.from({ length: pageCount }).map((_, index) => (
              <ToggleGroupItem
                key={index}
                className="cursor-pointer size-[32px] p-0 border text-gray-700 hover:bg-blue-100
                          data-[state=on]:bg-[#1e40af] data-[state=on]:text-white data-[state=on]:border-[#1e40af]"
      ...
      <div className="flex items-center space-x-2">
        <p className="text-[16px]">Tổng món ăn trong ngày: {totalRecords}</p>
      </div>
    </div>
  );
}
